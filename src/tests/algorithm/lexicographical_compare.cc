...
static size_t const I32_A_LEN = sizeof(I32_A) / sizeof(I32_A[0]);
static size_t const I32_B_LEN = sizeof(I32_B) / sizeof(I32_B[0]);
static size_t const U16_A_LEN = sizeof(U16_A) / sizeof(U16_A[0]);
static size_t const U16_B_LEN = sizeof(U16_B) / sizeof(U16_B[0]);
static size_t const STR_A_LEN = sizeof(STR_A) / sizeof(STR_A[0]);
...
public: /* constructor */
    context(
        t_ptr const a_array, size_t const a_array_len, t_ptr const b_array, size_t const b_array_len
    )
    ...
    , m_b_itr_2nd(m_b_ptr_2nd)
    , m_b_itr_end(m_b_ptr_end)
...
)
{
    t_status const results[] = {
        ...
        test_each_case(ratios, range0, range1, ctxt.m_eq),
        test_each_case(ratios, range0, range1, ctxt.m_ne),
        test_each_case(ratios, range0, range1, ctxt.m_lt),
        test_each_case(ratios, range0, range1, ctxt.m_le),
        test_each_case(ratios, range0, range1, ctxt.m_gt),
        test_each_case(ratios, range0, range1, ctxt.m_ge),
    };
...
inline static t_status test_each_range1(
    t_ratio_multiset &ratios, context<T> const &ctxt, range<InputIterator> const &range0
)
{
    ...
    t_ptr_range const ptr_ranges[RANGES_LEN] = {
        [EMPTY] = t_ptr_range(ctxt.m_b_ptr_1st, ctxt.m_b_ptr_1st),
        [ONLY1] = t_ptr_range(ctxt.m_b_ptr_1st, ctxt.m_b_ptr_2nd),
        [WHOLE] = t_ptr_range(ctxt.m_b_ptr_1st, ctxt.m_b_ptr_end),
    };
    t_itr_range const itr_ranges[RANGES_LEN] = {
        [EMPTY] = t_itr_range(ctxt.m_b_itr_1st, ctxt.m_b_itr_1st),
        [ONLY1] = t_itr_range(ctxt.m_b_itr_1st, ctxt.m_b_itr_2nd),
        [WHOLE] = t_itr_range(ctxt.m_b_itr_1st, ctxt.m_b_itr_end),
    };
    t_status const results[] = {
        test_each_overload(ratios, ctxt, range0, ptr_ranges[EMPTY]),
        test_each_overload(ratios, ctxt, range0, ptr_ranges[ONLY1]),
        test_each_overload(ratios, ctxt, range0, ptr_ranges[WHOLE]),

        test_each_overload(ratios, ctxt, range0, itr_ranges[EMPTY]),
        test_each_overload(ratios, ctxt, range0, itr_ranges[ONLY1]),
        test_each_overload(ratios, ctxt, range0, itr_ranges[WHOLE]),
    };
    size_t const results_len = sizeof(results) / sizeof(results[0]);
...
)
{
    ...
    t_status const results[] = {
        test_each_range1(ratios, ctxt, ptr_ranges[EMPTY]),
        test_each_range1(ratios, ctxt, ptr_ranges[ONLY1]),
        test_each_range1(ratios, ctxt, ptr_ranges[WHOLE]),

        test_each_range1(ratios, ctxt, itr_ranges[EMPTY]),
        test_each_range1(ratios, ctxt, itr_ranges[ONLY1]),
        test_each_range1(ratios, ctxt, itr_ranges[WHOLE]),
    };
    size_t const results_len = sizeof(results) / sizeof(results[0]);
...
{
    context<int32_t> const  i32_ctxt(&I32_A[0], I32_A_LEN, &I32_B[0], I32_B_LEN);
    context<uint16_t> const u16_ctxt(&U16_A[0], U16_A_LEN, &U16_B[0], U16_B_LEN);
    context<string> const   str_ctxt(&STR_A[0], STR_A_LEN, &STR_B[0], STR_B_LEN);

    t_status results[] = {
        test_each_range0(ratios, i32_ctxt),
        test_each_range0(ratios, u16_ctxt),
        test_each_range0(ratios, str_ctxt),
    };
    size_t const results_len = sizeof(results) / sizeof(results[0]);
...
}}} // namespace tester::algorithm::lexicographical_compare
